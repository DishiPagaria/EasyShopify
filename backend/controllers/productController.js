...
const productCreate = async (req, res) => {
    try {
        ...
        res.status(201).json(result); // Return HTTP status 201 for successful creation
    } catch (err) {
...
const getProductDetail = async (req, res) => {
    try {
        ...
        // Retrieve orders containing this product
        const orders = await Order.find({ 'orderedProducts._id': productId });
        console.log(orders);
        ...
        const productDetailWithOrders = {
            ...product.toObject(),
            orders: orderDetails
        };

        res.send(productDetailWithOrders);
    } catch (error) {
...
const getInterestedCustomers = async (req, res) => {
    try {
        ...
        }).populate('buyer', 'name'); // Populate the 'buyer' field to get the customer's name

        const customerDetails = orders.map(order => {
            const product = order.orderedProducts.find(item => item.productId.toString() === productId);
            if (product) {
                return {
            ...
            return null; // If product is not found in this order
        }).filter(item => item !== null); // Remove null values from the result
